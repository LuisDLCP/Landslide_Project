...
@author: Luis De La Cruz Painado
@email: luis11.dlcp@gmail.com 
"""
import libraries.imagingAlgorithm as BP
import libraries.sarPrm as sp
...
dir_input_files = root_path + "DataSet/Cuenca_Apr-Oct2021_test1/"
...
directory0 = root_path + "Results/" + dir_aux[-1] + "/"
...
def get_images(algorithm=None):
    ...
    create_SARimages_video()
    
    return 'Ok'

def create_SARimages_video():
    "Create a video from a set of SAR images"
    images_path = directory0 + "SAR_Images/Images/"
    video_path = directory0 + "SAR_Images/Videos/"
    if not os.path.exists(video_path):
        os.makedirs(video_path)
    cmd = "ffmpeg -framerate 50 -start_number 1 -i "+ images_path + "ImageBP_dset_%d.png -c:v libx264 -profile:v high -crf 20 -pix_fmt yuv420p "+ video_path + "SARimage.mp4"
    os.system(cmd)
...
def make_interferometry(data,algorithm=None):
    ...
    # Converting the set of interferograms into a video
    create_SARinterferometry_video()
...
def create_SARinterferometry_video():
    "Create a video from a set of SAR displacements images"
    images_path = directory0 + "Interferograms/Interferograms_complete/Images/"
    video_path = directory0 + "Interferograms/Interferograms_complete/Videos/"
    if not os.path.exists(video_path):
        os.makedirs(video_path)
    cmd = "ffmpeg -framerate 50 -start_number 0 -i "+ images_path + "Itf_BP_dset%d.png -c:v libx264 -profile:v high -crf 20 -pix_fmt yuv420p "+ video_path + "SARinterferogram.mp4"
    os.system(cmd)
...
def get_processing_time(start_time, end_time):
    delta_time = end_time - start_time # (s)
    if delta_time < 60:
        total_time = str(round(delta_time,2)) + ' s'
    elif delta_time < 3600:
        total_time = str(round(delta_time/60,2)) + ' m'
    else:
        total_time = str(round(delta_time/3600,2)) + ' h'
    
    h = open(directory0 + "/Statistical_graphs/Log_statistic.txt","a+")
    h.write("-----------------------------------------------------------------\n")
    h.write("Tiempo total de procesamiento: " + total_time)
    h.close()    
...
def main():
    ...
    get_processing_time(start_time=start_time, end_time=timeit.default_timer())
