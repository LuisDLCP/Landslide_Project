...
stations = ["Jicamarca","P.Maldonado","Tucumán","Tupiza"] # The first name will appear on top of Image 
...
def plot_ionogram(data_plot): 
    ...
    ax.set(xlabel=r"$[Days]$",ylabel=r"$[Stations]$")
    ...
    ax.set_xticks(np.arange(-0.5,len(dias),int(60*24/T)))#int(24/T)))
    ...
    # Create legend
    class1_box = mpatches.Patch(color=colores[0], label="Normal")
    class2_box = mpatches.Patch(color=colores[1], label="No data")
    class3_box = mpatches.Patch(color=colores[2], label="Zero size")
    class4_box = mpatches.Patch(color=colores[3], label="Same size")

    ax.legend(handles=[class1_box,class2_box,class3_box,class4_box],handlelength=0.9,bbox_to_anchor=(1.02,0.7),loc=2,borderaxespad=0.)
